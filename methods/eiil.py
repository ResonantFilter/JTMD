...
class EIILTrainer(BaseTrainer):
        ...
        for idx_epoch in range(args.bias_id_epoch):
            ...
            for batch, (dat, labels, _) in enumerate(pbar):
                ...
                pbar.set_description(
                    f"[{idx_epoch}/{args.bias_id_epoch}] loss: {meter.loss_avg.avg():.4f}"
                )
                # meter.log_epoch(idx_epoch)
                # meter.plot_subgroup_metrics(epoch=idx_epoch, save_dir="plots", show=False)
    ...
    def train(self):
        ...
        for batch, (dat, labels, _) in enumerate(pbar):
            ...
            accs = meter.compute_accuracy(output, obj_gt)
            losses = meter.compute_loss(output, obj_gt, torch.nn.CrossEntropyLoss(reduction="none"))
            meter.update(losses, accs, obj_gt, group_index)
